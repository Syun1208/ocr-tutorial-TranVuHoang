...
parser.add_argument('-train', '--trainroot', required=False, help='path to train dataset', default=os.path.join(WORK_DIR, 'data/naver-clova-ix-rec/output_train'))
parser.add_argument('-val', '--valroot', required=False, help='path to val dataset', default=os.path.join(WORK_DIR, 'data/naver-clova-ix-rec/output_validation'))
args = parser.parse_args()

args.trainroot = os.path.join(WORK_DIR, 'data/naver-clova-ix-rec/output_train')
args.valroot = os.path.join(WORK_DIR, 'data/naver-clova-ix-rec/output_validation')
    ...
    for epoch in range(params.nepoch):
        ...
        while i < len(train_loader):
            ...
            if i % params.displayInterval == 0:
                ...
                ctc_losses.append(float(loss_avg.val()))
                print(float(loss_avg.val()))
                loss_avg.reset()
            ...
            if i % params.saveInterval == 0:
                torch.save(crnn.state_dict(), '{}/CRNN_{}_{}.pth'.format(params.expr_dir, epoch, i))
    
    losses = pd.DataFrame({'ctc_losses': ctc_losses})
    losses.to_csv(os.path.join(path_results, 'losses.csv'))